...
class EDSR(object):
	...
	def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=3,output_channels=1):
		print("Building EDSR...")
		...
		x = slim.conv2d(self.input,feature_size,[3,3])
		...
		self.out = x#slim.conv2d(x,output_channels,[3,3])
		# self.out = tf.clip_by_value(output+mean_x,0.0,255.0)

		self.loss = loss = tf.reduce_mean(tf.losses.absolute_difference(self.target,self.out))
		...
		PSNR = tf.constant(20,dtype=tf.float32)*utils.log10(PSNR)
		# PSNR=compare_psnr(self.target, self.out,data_range=1)
		#inputs1=tf.image.resize_images(image_input,[img_size*scale,img_size*scale],method=0)

		#mse1 = tf.reduce_mean(tf.squared_difference(image_target, inputs1))
		#PSNR1 = tf.constant(255 ** 2, dtype=tf.float32) / mse1
		#PSNR1 = tf.constant(10, dtype=tf.float32) * utils.log10(PSNR1)
		# correct_prediction = tf.equal(tf.argmax(self.out, axis=-1), tf.argmax(self.target, axis=-1))
		# accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
	
		#Scalar to keep track for loss
		tf.summary.scalar("loss",self.loss)
		tf.summary.scalar("PSNR",PSNR)
		#tf.summary.scalar("PSNR_l_h",PSNR1)
	...
	"""
	def save(self,savedir='save1_models_128_20_re_20000'):
		print("Saving...")
	...
	def predict(self,x):
		...
		if (len(x.shape) == 3) and not(x.shape[0] == self.img_size and x.shape[1] == self.img_size):
			...
			tmp_image = np.zeros([x.shape[0]*self.scale,x.shape[1]*self.scale,1])
			fig=plt.figure()
			count=1
			for i in range(num_across):
				for j in range(num_down):
					a=fig.add_subplot(num_across,num_across,count)
