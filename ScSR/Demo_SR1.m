...
% read test image
img = load_untouch_nii('guqinying/20170521_1019393DT1TFE32chs501000a1005_brain.nii.gz');
im_o = img.img;
im_o=im_o(:,:,120);
%figure,imshow(im_o,[]);
...
im_l= imresize(im_h_mod, 1/3, 'bicubic');
...
maxIter =0;                   % if 0, do not use backprojection

% load dictionary
load('Dictionary/author/D_1024_0.15_5.mat');
% load('Dictionary/author/D_1024_0.15_5.mat');
...
sp_rmse = compute_rmse(im, im_rc);

bb_psnr = 10*log10(255/bb_rmse);
sp_psnr = 10*log10(255/sp_rmse);

fprintf('PSNR for Bicubic Interpolation: %f dB\n', bb_psnr);
fprintf('PSNR for Sparse Representation Recovery: %f dB\n', sp_psnr);


bb_ssim = metrix_ssim(im, im_b);
sp_ssim = metrix_ssim(im, im_rc);
